...
const createWorkflowValidation = [
  body('workflow_name')
    ...
    .trim()
    .matches(/^[A-Za-z0-9]+(?: [A-Za-z0-9]+)*$/).withMessage('Workflow name can only contain alphabets, numbers, and single spaces between words')
    .escape(),
  body('description')
    .optional()
...
const createWorkflowStepValidation = [
  body('step_name')
    ...
    .trim()
    .matches(/^[A-Za-z0-9]+(?: [A-Za-z0-9]+)*$/).withMessage('Step name can only contain alphabets, numbers, and single spaces between words')
    .escape(),
  body('step_order')
...
const updateWorkflowTransitionValidation = [
  body('transition_type')
    .optional()
    .isIn(['FORWARD', 'SENDBACK']).withMessage('Transition type must be FORWARD or SENDBACK')
    .trim(),
